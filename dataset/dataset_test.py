...
def generate_scaffolds(dataset, log_every_n=1000):
    ...
    print("length of dataset: ", data_len)
...
def scaffold_split(dataset, valid_size, test_size, seed=None, log_every_n=1000):
    ...
    train_cutoff = train_size * dataset.len()
    valid_cutoff = (train_size + valid_size) * dataset.len()
    train_inds: List[int] = []
    valid_inds: List[int] = []
    test_inds: List[int] = []
...
def read_smiles(data_path, target, task):
    ...
    print("length of smiles_data:", len(smiles_data))
    return smiles_data, labels


class MolTestDataset(Dataset):
    def __init__(self, data_path, target, task, transform=None, pre_transform=None, pre_filter=None, log=True):
        super().__init__(transform=transform, pre_transform=pre_transform, pre_filter=pre_filter, log=log)
        self._indices = None
